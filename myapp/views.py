...
class PeoplePageView(LoginRequiredMixin, StaffRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        objectlist['admin'] = CustomUser.objects.filter(
            groups__name='adminusers')
        objectlist['customer'] = CustomUser.objects.filter(
            ...
            for image in formset.forms:
                ...
                if image.is_valid():
                    if image.cleaned_data.get('image'):
                        ...
                        image.instance.save()
                        image.save()
                     # to delete the exisiting image
                    elif image.cleaned_data.get('image') is False:
